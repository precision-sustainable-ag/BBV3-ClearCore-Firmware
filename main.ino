// 9 Jan 2024  ver. 0.04 : Two sensors for X-axis work good
...
// (in Linux one can use netcat: nc -u 192.168.0.121 8888)
// Format of a message: X:number Z:number
// number is an integet, can be negative
// Also works: Z:number X:number

// 0-th motor controls X-axis (horiz.), 1-st motor controls Z-axis (vertical)
// Eventually program should be ready to work with up to 4 motors
...
int velocityLimit = 3000; //10000; // pulses per sec
...
void loop() {
    ...
    if (packetSize > 0) {
        ...
        //Udp.write(itoa(dist, dist_char, 10) );  // int-to-char convertion
        ...
        Udp.write(itoa(dist_Z, Z_dist_char, 10) );  // int-to-char convertion
        ...
        int dist_0 = dist_X;
        int dist_1 = dist_Z;
        motor_0_MoveDistance(dist_0);
        //delay(10); // wait 5 sec for motor_0_MoveDistance(dist_0) to finish
        motor_1_MoveDistance(dist_1 + dist_1); // starting 2nd motor when the first stopped
        delay(10); // actually, this works without a delay
